...
| **F-B04** | **取引一覧と検索** | **目的:** 過去の実績を含む全ての取引履歴の確認や、特定の取引の検索を容易にする。<br>**検索・フィルタ・ソート:**<br>1. **検索フォーム:** 画面上部に配置。メモ欄のキーワード検索と、開始日・終了日による期間検索が可能。片方の日付が空でも検索できる。<br>2. **フィルタ:** 検索フォーム下にアコーディオンで配置。口座、カテゴリ、ステータス（予定/完了）をボタン形式で複数選択可能。選択された条件はANDで結合される。<br>3. **ソート:** リストの日付ヘッダー横のボタンで、昇順・降順を切り替える。<br>**リスト表示:**<br>1. **表示項目:** 日付、カテゴリ、予定額、実績額、ステータス、メモ、編集/削除ボタン。<br>2. **ページネーション:** 1ページあたり50件のレコードを表示する。<br>**関連データ:** `transactions` (READ), `accounts`(READ), `categories`(READ) |
...
### 3.1. 共通レイアウトとThymeleafフラグメント戦略
- **基本方針:** アプリケーション全体のUIの一貫性とメンテナンス性を高めるため、Thymeleafのレイアウトデコレーターパターンを採用する。
- **共通レイアウトファイル (`layout/app.html`):** 全てのページの土台となるHTML構造（`<html>`, `<head>`, `<body>`）、共通CSS、共通JavaScriptを定義する。このファイルは、各ページから渡される複数のフラグメント（部品）を適切な位置に挿入する役割を持つ。
- **フラグメントファイル (`fragments/`):** ヘッダー（`_header.html`）やサイドバー（`_sidebar.html`）など、再利用可能なUI部品を個別のファイルとして定義する。
- **各ページの役割:** `dashboard.html`などの各ページは、共通レイアウト(`app.html`)に対して、自身のタイトル、パンくずリスト、メインコンテンツ、モーダル、専用スクリプトといった部品をフラグメントとして提供する責務を持つ。

#### **【重要】モーダル実装に関する注意点**
Thymeleafのレイアウトデコレーターを利用する際、メインコンテンツのフラグメント(`th:fragment="content"`)の中に、モーダルのような別のフラグメント(`th:fragment="modals"`)を定義すると、テンプレートの解決エラーが発生する場合がある。
これを回避するため、**各ページは`content`、`modals`、`scripts`といった複数のフラグメントを個別に定義し、`app.html`側でそれぞれをパラメータとして受け取り、適切な場所に挿入する設計とする。**
...
| **取引一覧** | `/budget/transactions` | 全ての取引（過去の実績含む）を詳細に確認・管理する。**検索、フィルタ、ソート、ページネーション機能を持つ。** | `F-B04`, `F-B03`, `F-B13` | `transactions` |
